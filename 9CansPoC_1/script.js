...
function setupButtons() {
  ...
  button.mousePressed(toggleDemo);
  yPos-=40;
  button = createButton(' SCAN ');
  button.position(xPos,yPos);
  button.mousePressed(toggleScan);
  yPos-=40;
  button = createButton(' TRACK ');
  button.position(xPos,yPos);
  button.mousePressed(toggleTrack);
  yPos-=40;
  button = createButton(' 9 CANS ');
  button.position(xPos,yPos);
  button.mousePressed(toggleCans);
}
...
drawJoystick();
 ...
 fill(color(255, 255, 255));
  ...
  if (player1.score == gameRound * 9 && drawTrack==false) { newCans(); } 
    updateTimer();
}
